...
def read(file_name):
	...
	while i < len(lines):
		...
		if "<ipxact:adHocConnection>" in line:
			Modules[nModules].nConnections += 1
			Modules[nModules].Connections.append(Connections())
			while "</ipxact:adHocConnections>" not in line:
				...
				if "<ipxact:internalPortReference " in line:
					...
					module_connections.append(p.search(line))
					j = 0
					for module in Modules:
						if module.Name == module_connections[-1].group(1):
							break
						j += 1
					i += 1
					line = lines[i]
